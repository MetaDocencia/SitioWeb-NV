...
---
title: "RReporte 2022. Un año de cambios y crecimiento."
...
paises <- world %>% 
  ...
  mutate(postulaciones = ifelse(is.na(n), "No", "Si"),
         Etiqueta = ifelse(postulaciones == "Si", str_c(cldr.name.es, "\n", n, " asistentes"), " "),
         Etiqueta2 = ifelse(postulaciones == "Si", str_c(cldr.name.es, " (", n, ")"), NA))
...
Luego de la primera búsqueda laboral abierta de nuestra historia, incorporamos [nuevas personas y roles](https://www.metadocencia.org/post/mdcrece/), reorganizando tareas y funciones en los siguientes equipos de trabajo, que siguen funcionando en 2023 (link a cada perfil solo la primera vez que se nombra)🗃️:
...
gg <- ggplot(paises) +
  ...
  labs(title = "Origen de quienes asistieron a los eventos de gobernanza") +
...
post %>% 
  count(`¿Cómo te enteraste de estos encuentros?`) %>% 
  mutate(n = n/sum(n)*100) %>% 
  ggplot(aes(x = `¿Cómo te enteraste de estos encuentros?`, y = n)) +
